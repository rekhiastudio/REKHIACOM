...
export const NavItems = ({ items, className, onItemClick }: NavItemsProps) => {
  ...
  return (
    <motion.div
      ...
      // ! HABIA UN ABSOLUTE Y UN JUSTIFY-CENTER AQUI
      className={cn(
        "inset-0 hidden flex-1 flex-row items-center  space-x-2 text-sm font-medium text-zinc-600 transition duration-200 hover:text-zinc-800 lg:flex lg:space-x-2",
        className,
      )}
    >
      {items.map((item, idx) => (
  ...
  return (
    ...
    >
      ...
      <div className="pt-3 ml-5">
        <span className={`${notable.className} text-xl text-white`}>REKHIA</span>
      </div>
    </Link>
  );
};
